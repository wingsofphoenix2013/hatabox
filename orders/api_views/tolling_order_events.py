...
from orders.serializers import TollingOrderEventSerializer


class TollingOrderEventViewSet(ReadOnlyModelViewSet):
    queryset = TollingOrderEvent.objects.select_related(
        "order",
        "created_by",
    ).order_by("-created_at", "-id")
    serializer_class = TollingOrderEventSerializer

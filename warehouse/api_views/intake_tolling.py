...
from warehouse.services.intake_tolling import (
    accept_tolling_receipt_item_to_location,
    bulk_accept_tolling_receipt_items_to_location,
)
...
class WarehouseTollingPendingIntakeItemViewSet(ReadOnlyModelViewSet):
    ...
    serializer_class = WarehouseTollingPendingIntakeItemSerializer
    ...
    def get_queryset(self):
        ...
        if inventory_item:
            queryset = queryset.filter(order_item__inv_item_id__in=inventory_item)

        inventory_item_id = self.request.query_params.get("inventory_item_id")
        if inventory_item_id:
            queryset = queryset.filter(order_item__inv_item_id=inventory_item_id)
        ...
        if search:
            queryset = queryset.filter(
                ...
                | models.Q(order_item__order__organization__name__icontains=search)
    ...
    def accept_to_location(self, request, pk=None):
        ...
        result = accept_tolling_receipt_item_to_location(
    ...
    def bulk_accept_to_location(self, request):
        ...
        result = bulk_accept_tolling_receipt_items_to_location(
